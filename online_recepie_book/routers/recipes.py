...
from online_recepie_book.models.recipe import Recipe,   RecipeCreate
from online_recepie_book.database import get_db_connection
...
def update_recipe(recipe_id: int, recipe: RecipeCreate):
    if not category_exists(recipe.category_id):
        raise HTTPException(status_code=400, detail="category dose not exist")
    conn = get_db_connection()
    cursor = conn.cursor()
    cursor.execute("UPDATE recip set name=?, description=?, instruction=?, ingredients=?, cuisine=?, difficulty=?, category_id=? WHERE id=?",
                   (
                       recipe.name,
                       recipe.description,
                       recipe.ingredients,
    ...
    if cursor.rowcount == 0:
        conn.close()
        raise HTTPException(status_code=404, detail="recipe not fount")
    conn.commit()
    conn.close()
