...
if selected_menu == "dashboard":
    ...
    if category_list:
        ...
        selected_category_id = next((cat['id'] for cat in category_list if cat['name'] == selected_category_name), None)
    else:
        st.error("Failed to retrieve categories.")
        selected_category_id = None

    if st.button("Add Recipe", key="add_recipe_button"):
        if all([recipe_name, recipe_description, recipe_ingredients, recipe_instructions, recipe_cuisine, recipe_difficulty]) and selected_category_id is not None:
            ...
            st.error("All fields must be filled and a category selected to add a recipe.")
    ...
    if recipe_list:
        ...
        if manage_action == "edit":
            ...
            if recipe_to_edit:
                ...
                edit_name = st.text("recipe name", value= selected_recipe['name'], key="edit_recipe_name")
                ...
                if category_list:
                    ...
                    edit_category_name = st.selectbox(
                        ...
                        index=category_name.index(
                            next(cat['name'] for cat in category_list if cat['id'] == selected_recipe['category'])
                        )
                    )
                    edit_category_id = next(cat['id'] for cat in category_list if cat['name'] == edit_category_name)
                else:
                    st.error("failed to retrieve category.")
                ...
                if st.button("update recipe", key="update_recipe_button"):
                    if all(edit_name, edit_description, edit_ingredients, edit_instructions, edit_cuisine, edit_difficulty, edit_category_id is not None):
                        ...
                        st.error("all feilds must be filled and category selected to update a recipe.")
